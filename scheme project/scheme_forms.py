...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        fnct_name = signature.first
        formals = signature.rest
        body = expressions.rest
        my_lambda = do_lambda_form(Pair(formals, body), env)
        env.define(fnct_name, my_lambda)
        return fnct_name
        # END PROBLEM 10
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while expressions.rest is not nil:
            evaluated_expr = scheme_eval(expressions.first, env)
            if is_scheme_true(evaluated_expr):
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    seen_symbols = set()  # Set to keep track of seen symbols
    # BEGIN PROBLEM 14
    while bindings is not nil:
        ...
        if not scheme_symbolp(symbol):
            raise SchemeError('invalid symbol in binding: {0}'.format(symbol))
        if symbol in seen_symbols:
            raise SchemeError('duplicate binding for symbol: {0}'.format(symbol))
